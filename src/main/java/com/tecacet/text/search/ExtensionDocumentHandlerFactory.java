...
public class ExtensionDocumentHandlerFactory implements DocumentHandlerFactory {
    ...
    private final Map<String, DocumentHandler> handlers = new HashMap<>();

    private Logger log = LoggerFactory.getLogger(this.getClass());

    public ExtensionDocumentHandlerFactory() throws IOException, DocumentHandlerException {
        this(DEFAULT_CONFIG_FILE);
    }

    public ExtensionDocumentHandlerFactory(String propertiesFile) throws IOException, DocumentHandlerException {
    ...
    public ExtensionDocumentHandlerFactory(Map props) throws DocumentHandlerException {
        for (Object key : props.keySet()) {
            String ext = (String) key;
    ...
    public void addHandler(String extension, String handlerClassName) throws DocumentHandlerException {
    ...
    @Override
    public DocumentHandler getHandlerForFile(File file) throws DocumentHandlerException {
    ...
    private DocumentHandler instantiateHandler(String handlerClassName) throws DocumentHandlerException {
        try {
            Class<?> handlerClass = Class.forName(handlerClassName);
            DocumentHandler handler = (DocumentHandler) handlerClass.newInstance();
            return handler;
        } catch (ClassNotFoundException e) {
            throw new DocumentHandlerException(String.format(CANNOT_CREATE_HANDLER_MESSAGE, handlerClassName), e);
